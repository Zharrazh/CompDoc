...
export const DataGrid = <TTemplates extends TemplateInfo<string, any>>({
  ...
  const [scroll, setScroll] = useState<ScrollInfo>({ x: 0, y: 0, height: 0 });

  const unlockedRef = useRef<HTMLDivElement>(null);

  useLayoutEffect(() => {
    if (unlockedRef.current != null) setScroll({ x: 0, y: 0, height: unlockedRef.current.clientHeight });
  }, [unlockedRef]);
  ...
  const [lockedWidth, lockedHeight] = useMemo(
    () => getLockedWidthAndHeight(lockedColumns, lockedRows, columns, templates, rows),
    [lockedColumns, lockedRows, columns, templates, rows]
  );
  ...
  const resizeCallback = useCallback(() => {
    if (unlockedRef.current != null && scroll.height !== unlockedRef.current.clientHeight)
      setScroll({ ...scroll, height: unlockedRef.current.clientHeight });
  }, [scroll, unlockedRef]);
    ...
    for (let k = 0, l = 0; k < columns.length && l < columnCount; k++, l++) {
      ...
      if (k + span > columnStart) {
        ...
        if (span > 1 ? isGroupVisible(columns, k, span) : isVisible(column)) {
          const width = getCellWidth(columns, k, span);
          renderedCells.push(
  ...
  return (
    <div
      className={classNames(
        'dataGrid-row',
        typeof rowInfo.template.classNames === 'string'
          ? rowInfo.template.classNames
          : rowInfo.template.classNames?.(rowInfo.row)
      )}
