...
type MultiselectProps = {
  ...
  readonly defaultSelectedId?: number[];
  onChange: (selectedOption: Option[]) => void;
};

export const Multiselect: FunctionComponent<MultiselectProps> = ({ options, defaultSelectedId, onChange }) => {
  ...
  const createOnDropItemCallback = (option: Option) => () => {
    ...
    const newSelectedOptions = selectedOptions.filter(o => o !== draggableItem);
  ...
  let focusTimer: NodeJS.Timeout | number;
  const handleOnFocus = () => {
    clearTimeout(focusTimer as NodeJS.Timeout);
    if (!isFocused) {
      setFocused(true);
    }
  };
...
const OptionListItem: FunctionComponent<OptionListItemProps> = ({ option, onClick }) => {
  return (
    <div className={'optionListItem'} onClick={onClick}>
...
const SelectedOptionItem: FunctionComponent<SelectedOptionItemProps> = ({
  option,
  onDeleteClick,
  onDragStart,
  onDragEnd
...
const Dropzone: FunctionComponent<DropzoneProps> = ({ onDropItem, isDraggingNow }) => {
  ...
  return (
    <div className={'dropZoneWrapper'}>
      <div
        className={cn('dropZone', { display: isDisplay, checkMode: isDraggingNow })}
